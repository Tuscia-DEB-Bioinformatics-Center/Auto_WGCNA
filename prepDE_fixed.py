...
parser.add_option('-s', '--string', default = "STRG", help = "if a different prefix is used for geneIDs assigned by StringTie [default: %default]")
...
if os.path.isfile(opts.input):
    ...
    try:
        ...
        for line in fin:
            if line[0] != '#':
                lineLst = tuple(line.strip().split('\t', 2))
                if len(lineLst) != 2:
...
for q, s in enumerate(samples):
    ...
    for i, v in t_dict.items():
        ...
        try:
            ...
            # geneDict[geneIDs[i]][s[0]] += v.get(s[0], 0)

            total_v += v.get(s[0], 0)
            if i not in geneIDs:
                skipped_n += 1
                skipped_v += v.get(s[0], 0)
                continue

            geneDictTemp = geneDict.get(geneIDs[i], {s[0]: 0})
            geneDictTemp[s[0]] = geneDictTemp.get(s[0], 0) + v.get(s[0], 0)
            geneDict[geneIDs[i]] = geneDictTemp
        except KeyError:
...
if skipped_n > 0:
    print(f'[INFO] Skipped {skipped_n} entries (total value: {skipped_v} = {round(skipped_v / total_v * 100, 1)}%)')
